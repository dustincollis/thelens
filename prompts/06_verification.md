---
name: verification
description: Checks a page-aware LLM response against the actual page text and flags claims that are not supported. Run once per provider response.
...
- `support_level: paraphrased` means the claim restates page content in different words. NOT a hallucination.
- `support_level: partially_supported` means part of the claim is on the page but part is added.
- `support_level: opinion_or_inference` means the claim is the evaluator's judgment, not a factual claim about the page. Subjective ratings (e.g., "self-sufficiency: 6") and lists of issues that are evaluator critique are inferences. Do NOT flag these as hallucinations.
...
- `overall_support_level` aggregates the field checks. Use `fully_supported` if all fields are supported, paraphrased, or opinion_or_inference; down to `unsupported` if multiple fields contain hallucinations.
