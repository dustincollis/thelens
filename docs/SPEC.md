...
A single-file `src/thelens/app.py` that wraps the pipeline in a browser form. Launched with `streamlit run src/thelens/app.py` or `lens dashboard`. This is the primary user-facing surface — once it ships, the CLI becomes optional.

The page contains:

- A text input for the URL and a **Run** button.
- While a run is in progress, a status area showing the current pipeline step. This is per-step refresh (the UI updates as each step finishes), not true streaming progress.
- When the run finishes, a link to the generated HTML report.
- Below the form, a list of recent runs from the SQLite index (most recent first), each with an "Open" link to its HTML report.

The UI is a thin wrapper. It calls the same `run_pipeline()` function the CLI calls; no business logic lives in the UI layer. Concurrency is intentionally simple: if the user submits a second URL while one is running, the form is disabled until the current run completes.

Delivered in two phases:

- **Phase 6** ships the form, run, and "open report" link.
- **Phase 8** adds the recent-runs list and any later polish.
...
### Phase 6: Streamlit web UI (run + view)

- Implement `src/thelens/app.py` as a minimal single-page Streamlit app: URL input, Run button, per-step status updates while running, link to the HTML report when finished.
- Refactor `pipeline/run.py` if needed so `run_pipeline()` is callable from non-CLI code (it should already be, but verify).
- Wire `lens dashboard` in `cli.py` to launch Streamlit on the bundled `app.py`.

**Acceptance:** `lens dashboard` opens a browser page. Entering a URL and clicking Run produces a complete run folder and opens the HTML report. The CLI `lens run <url>` still works and produces identical artifacts.
...
- `--cache-fetch` flag
- Add the recent-runs list to `app.py`
- Tests in `tests/` for `audit.py`, `models.py`, `storage.py`

**Acceptance:** `lens estimate <url>` prints a projected cost. The web UI shows recent runs below the form. Test suite passes.
