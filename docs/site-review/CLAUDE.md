...
2. Build in the phases defined in `SPEC.md` section 15. Do not skip phases.
...
- Load prompts via a single helper in `src/site_review/llm/base.py` that reads the file, splits the frontmatter, and returns a structured object.
...
- Use `asyncio.Semaphore(4)` to cap parallelism at 4 concurrent API calls regardless of how many providers and questions are configured.
...
- Do not invent new pipeline steps. The eight steps in `SPEC.md` section 3 are the entire pipeline.
