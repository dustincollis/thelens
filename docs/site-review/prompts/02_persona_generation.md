...
You generate personas that are useful for finding problems, not for celebrating the site. Personas must conflict with each other on at least one dimension so that the resulting reviews surface genuine tensions, not redundant agreement.

You return JSON only, with no preamble.

# User

Below is the structured fingerprint of a website. Generate 3 to 5 review personas that would each provide a distinct, useful critical lens on this specific site.
  ...
  "personas": [
    {
      ...
      "is_llm_lens": "boolean, true for the AI-as-reader persona, false for human personas",
...
- The LLM-as-reader persona is required. Its `name` should be a plausible AI assistant name (e.g., "Aria", "Atlas") and its `role` should be "AI assistant retrieving this page in response to a user query."
- For the LLM-as-reader persona, `expertise_level` is "expert" and `trust_posture` is "neutral."
- Do not generate two personas that share the same primary concerns. If you find yourself doing this, replace one with a different lens.
